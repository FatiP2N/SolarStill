# -*- coding: utf-8 -*-
"""SolarStill_colab.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Zb9eXXgx3BlaUbe4DWur49eCd-soPQEb
"""
...
"""Install IAPWS an GSPREAD and make sure that the excel input files are imported before runing the code"""
...
##############################################################################
    ...
    def cpk(self, me, i=0.02, Svg=1e-3,Svb=1e-4,Svi=1e-3):
        ...
        daily_cost = AC_total / 275  # mean number of sunny days in Morocco
        CPK = daily_cost / me
        return CPK
...
# computes the function x -> x/2 + log(a+b.x) which zero corresponds to 1/sqrt(f_D)
    ...
    def Bilan(self,t,Tan,Ta0,Ig0):
        ...
        #Evaporated water
        cm=0
        bm=self.conv*(self.aw*self.hvap_(Tg0,Tw0)*self.etoile(Tw0-Tg0))/(self.Lv(Tw0))
        dm=0
    ...
    def store_data(self, store=False):
        ...
        if store.lower() == "yes":
            ...
            if not math.isnan(self.Texp[(len(self.t) - 1), 3]):
                tab2 = [self.device + " SS " + gd14, self.mater, self.ns, self.ab, self.teta1, self.teta2, self.Texp[len(self.t) - 1, 3], self.exp_efficiency / 100, self.cpkexp, np.mean(self.ta) - self.kelv, max(self.ta) - self.kelv, min(self.ta) - self.kelv, np.mean(self.ig), max(self.ig), min(self.ig)]
                tab1 = [self.device + " SS " + gd14, self.mater, self.ns, self.ab, self.teta1, self.teta2, self.me, self.efficiency / 100, self.cost, np.mean(self.ta) - self.kelv, max(self.ta) - self.kelv, min(self.ta) - self.kelv, np.mean(self.ig), max(self.ig), min(self.ig)]
                self.data = [tab1, tab2]
            else:
                tab1 = [self.device + " SS " + gd14, self.mater, self.ns, self.ab, self.teta1, self.teta2, self.me, self.efficiency / 100, self.cost, np.mean(self.ta) - self.kelv, max(self.ta) - self.kelv, min(self.ta) - self.kelv, np.mean(self.ig), max(self.ig), min(self.ig)]
                self.data = [tab1]
            ...
            columns = ['Type', "mat", "ns", "Ab", "tetag", "tetas", 'daily yield', 'efficiency', 'Cost', 'Ta(mean)', 'Ta(max)', 'Ta(min)', 'Ig(mean)', 'Ig(max)', 'Ig(min)']
